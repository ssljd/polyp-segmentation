...
class Res2UNet(nn.Module):
    def __init__(self, img_ch=3, output_ch=1, channel=32):
    ...
    def forward(self, x):
        ...
        # ---- low-level features ----
        x_1 = self.resnet.layer1(x)      # bs, 256, 88, 88
        x_2 = self.resnet.layer2(x_1)     # bs, 512, 44, 44

        x_3 = self.resnet.layer3(x_2)     # bs, 1024, 22, 22
        x_4 = self.resnet.layer4(x_3)     # bs, 2048, 11, 11

        x2_rfb = self.rfb2_1(x_2)  # channel -> 32
        x3_rfb = self.rfb3_1(x_3)  # channel -> 32
        x4_rfb = self.rfb4_1(x_4)  # channel -> 32

        ra_feat = self.agg1(x4_rfb, x3_rfb, x2_rfb)
        lateral_map = F.interpolate(ra_feat, scale_factor=8, mode='bilinear')

        e1 = self.Conv1(lateral_map)
